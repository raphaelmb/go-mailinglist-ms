...
func UpdateEmail(db *sql.DB, entry EmailEntry) error {
	...
	_, err := db.Exec(`INSERT INTO emails(email, confirmed_at, opt_out) VALUES(?, ?, ?) ON CONFLICT(email) DO UPDATE SET confirmed_at=? opt_out=?`, entry.Email, t, entry.OptOut, t, entry.OptOut)
	if err != nil {
		log.Println(err)
		return err
	}
...
func GetEmailBatch(db *sql.DB, params GetEmailBatchQueryParams) ([]EmailEntry, error) {
	...
	rows, err := db.Query(`SELECT * FROM emails WHERE opt_out = false ORDER BY id ASC LIMIT ? OFFSET ?`, params.Count, (params.Page-1)*params.Count)
	if err != nil {
		log.Println(err)
		return empty, nil
	}
